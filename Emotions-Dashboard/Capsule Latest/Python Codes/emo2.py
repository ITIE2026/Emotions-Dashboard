...
def send_mental_data_objects_over_serial(mental_data, port='COM3', baudrate=9600):

    try:
        with serial.Serial(port, baudrate, timeout=.1) as ser:
            
            for item in mental_data:
                # Extract numeric values
                if all(hasattr(item, attr) for attr in ['rel_attention', 'rel_relaxation', 'inst_attention', 'inst_relaxation']):
                    values = [item.rel_attention, item.rel_relaxation, item.inst_attention, item.inst_relaxation]
...
def on_signal_received(sensor, data):
    ...
    if not math.calibration_finished():
        ...
        if len(mental_data) > 0:
            ...
            send_mental_data_objects_over_serial(mental_data, port='COM3', baudrate=9600)
